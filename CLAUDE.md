...
- **`lib/clipboardPopup.js`** — all the `St` UI. Uses the Shell's `GrabHelper` for the modal grab so Esc / click-outside / focus-loss close cleanly; don't hand-roll grabs (risks locking the session). Keyboard: typing filters, ↑/↓ move, Enter selects, Delete removes (Shift+Delete while a search query is active), Esc closes. Rows show a whitespace-collapsed, truncated preview but the store keeps/returns full text.
- **`lib/indicator.js`** — optional panel button; a **primary-button** click calls back into `extension.js` to open the popup anchored below the icon (other buttons pass through).

## Cross-cutting constraints

- **Super+V conflict:** GNOME binds `<Super>v` to `toggle-message-tray` (`['<Super>v','<Super>m']`). `enable()` strips `<Super>v` from that key (leaving `<Super>m`) **only while Clippo is actually bound to `<Super>v`**; `_reconcileTrayConflict()` re-checks on every `changed::toggle-clippo` and hands it back the moment Clippo stops using it, and `disable()` restores it too. Any change to keybinding setup must preserve restore-on-disable, or the user permanently loses Super+V for the message tray.
...
- **v1 is text-only.** Copies a password manager flags as secret (`x-kde-passwordManagerHint`) are skipped, but passwords copied from anywhere else are still captured like normal text (mitigated only by the 0600 file). No full private mode yet.
