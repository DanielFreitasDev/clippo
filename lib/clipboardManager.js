...
// Mime-type hint that password managers (KeePassXC, KDE, browsers…) attach to a
// clipboard copy to mark it as a secret. When present, we skip capturing it.
    ...
    start() {
        ...
        this._ownerChangedId = this._selection.connect('owner-changed',
            (_selection, selectionType) => {
                ...
                if (this._isPassword())
                    return; // don't store secrets flagged by a password manager
                this._readClipboard();
            });
    }

    // True when the current clipboard owner marked its content as a password, so
    // we leave it out of the history entirely. Best-effort: if the API is missing
    // or throws, we fall back to capturing as usual.
    _isPassword() {
        try {
            const mimetypes = this._selection.get_mimetypes(
                Meta.SelectionType.SELECTION_CLIPBOARD);
            return !!mimetypes && mimetypes.includes(PASSWORD_HINT_MIME);
        } catch (_e) {
            return false;
        }
    }
