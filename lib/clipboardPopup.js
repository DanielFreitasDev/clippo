...
import { detectSubtype, colorValue, openWithType } from './contentType.js';
...
export const ClipboardPopup = GObject.registerClass({
    Signals: {
        ...
        'open-with-invoked': { param_types: [GObject.TYPE_STRING, GObject.TYPE_STRING] },
        'clear-requested': {},
    },
    ...
    _buildUI() {
        ...
        this._pinnedToggle = new St.Button({
            style_class: 'clippo-icon-button',
            toggle_mode: true,
            child: new St.Icon({ icon_name: 'starred-symbolic', icon_size: 16 }),
        });
        this._pinnedToggle.accessible_name = _('Show pinned only');
        this._pinnedToggle.connect('clicked', () => {
            this._pinnedOnly = this._pinnedToggle.checked;
            this._applyFilter();
        });
        ...
        header.add_child(this._pinnedToggle);
    ...
    _rebuildRows() {
        ...
        // Shown by _applyFilter() when the search/pinned filter matches nothing.
        ...
        this._listBox.add_child(this._noResults);
    }

    _buildRow(entry) {
        const row = new St.BoxLayout({ style_class: 'clippo-row', x_expand: true });
        row._id = entry.id;
        row._pinned = !!entry.pinned;
    ...
    _openDetail(id) {
        ...
        if (entry.type === 'text') {
            ...
            if (sub === 'url' || sub === 'email') {
                ...
                addAction(_('Open With…'), () => this._openWith(id));
            }
        }
        addAction(_('Remove'), () => this._requestRemove(id));
        this._detailView.add_child(actions);

        this._switchToDetail(actions.get_first_child());
    }

    // Lists the installed apps that can open this item (url/email) and lets the
    // user pick one. The chosen app is launched by extension.js via
    // 'open-with-invoked', keeping app-launching out of this UI module.
    _openWith(id) {
        const entry = this._entryById(id);
        if (!entry || entry.type !== 'text')
            return;
        const subtype = this.detectTypes ? detectSubtype(entry.text) : null;
        const type = openWithType(subtype, entry.text);
        if (!type)
            return;

        this._mode = 'openwith';
        this._detailView.destroy_all_children();
        this._detailView.add_child(this._detailTopbar(_('Open With')));

        const apps = this._appsForType(type);
        const scroll = new St.ScrollView({ style_class: 'clippo-detail-scroll', y_expand: true });
        const list = new St.BoxLayout({
            orientation: Clutter.Orientation.VERTICAL,
            style_class: 'clippo-openwith-list',
        });
        if (apps.length === 0) {
            list.add_child(new St.Label({
                style_class: 'clippo-empty',
                text: _('No application available'),
                x_align: Clutter.ActorAlign.CENTER,
            }));
        }
        for (const app of apps) {
            ...
            if (!appId)
                continue;
            const btn = new St.Button({ style_class: 'clippo-openwith-item', x_expand: true });
            const box = new St.BoxLayout({ style_class: 'clippo-openwith-row' });
            ...
            btn.set_child(box);
            btn.connect('clicked', () => {
                this.emit('open-with-invoked', id, appId);
                this.dismiss();
            });
            list.add_child(btn);
        }
        this._setScrollChild(scroll, list);
        this._detailView.add_child(scroll);

        this._switchToDetail(scroll);
    }

    // The apps that can open a content type, recommended ones first.
    _appsForType(type) {
        try {
            let apps = Gio.AppInfo.get_recommended_for_type(type);
            if (!apps || apps.length === 0)
                apps = Gio.AppInfo.get_all_for_type(type);
            return apps ?? [];
        } catch (_e) {
            return [];
        }
    ...
    // Removes an item, asking first when it is pinned (pinned items are meant to
    // stick around, so removing one shouldn't be a single misclick).
    _requestRemove(id) {
        const entry = this._entryById(id);
        if (!entry)
            return;
        ...
        this._mode = 'confirm';
        this._detailView.destroy_all_children();
        this._detailView.add_child(this._detailTopbar(_('Remove Pinned Item?')));

        const msg = new St.Label({
            style_class: 'clippo-confirm-text',
            text: _('This item is pinned. Remove it from the history anyway?'),
        });
        msg.clutter_text.set_line_wrap(true);
        this._detailView.add_child(msg);

        const actions = new St.BoxLayout({ style_class: 'clippo-detail-actions' });
        const remove = new St.Button({ style_class: 'clippo-detail-button', label: _('Remove') });
    ...
    _applyFilter() {
        ...
        for (const row of this._rows) {
            const matchesQuery = query === '' || row._searchText.includes(query);
            const matchesPinned = !this._pinnedOnly || row._pinned;
            const match = matchesQuery && matchesPinned;
            row.visible = match;
            if (match)
        ...
        if (this._noResults)
            this._noResults.visible = this._visibleRows.length === 0;
    ...
    _onKeyPress(event) {
        ...
        case Clutter.KEY_Delete: {
            ...
            if (this._highlight >= 0 && (shift || !editing)) {
                this._requestRemove(this._visibleRows[this._highlight]._id);
                return Clutter.EVENT_STOP;
            }
    ...
    show({ x, y }) {
        ...
        this._pinnedOnly = false;
        if (this._pinnedToggle)
            this._pinnedToggle.checked = false;
        this._applyFilter();
