// clipboardPopup.js
//
// All of the popup's St UI: search bar, scrollable item list, pinned items,
// keyboard navigation and dismissal (selection / Esc / click-outside / focus
// loss). The popup is placed at the position passed in (pointer or below the
// icon).
//
...
const PREVIEW_MAX = 120; // characters shown per item (the full text is kept)
...
export const ClipboardPopup = GObject.registerClass({
    ...
    _onKeyPress(event) {
        ...
        case Clutter.KEY_Delete: {
            // Remove the highlighted item, but don't hijack forward-delete while
            // the user is editing a search query — Shift+Delete forces removal.
