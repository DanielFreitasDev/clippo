...
export class HistoryStore {
    ...
    load() {
        if (!this.persist) {
            ...
            // previous (persisting) session may have left on disk.
            this._entries = [];
            this._purgeDisk(true);
            return;
        }
    ...
    // Switches persistence on or off at runtime. Turning it on writes the
    // current in-memory state out; turning it off drops the on-disk text index
    // immediately (images shown this session stay until logout).
    setPersist(on) {
        if (this.persist === on)
            return;
        this.persist = on;
        if (on)
            ...
            this._purgeDisk(false);
    }

    // Removes the history from disk. With includeImages, also empties the images
    // directory (used at startup and teardown); without it, only the JSON index
    // is deleted (used when persistence is switched off mid-session, so images
    // still on screen keep working).
    _purgeDisk(includeImages) {
        try {
            const file = Gio.File.new_for_path(this._path());
    ...
    save() {
        ...
        try {
            ...
            // PRIVATE creates that temp file as 0600 from the start, so the data
            // is never briefly world-readable (the history may hold secrets).
    ...
    // not persisting, wipes the on-disk history instead so nothing survives.
    destroy() {
        if (this._saveSourceId) {
            GLib.Source.remove(this._saveSourceId);
            this._saveSourceId = 0;
